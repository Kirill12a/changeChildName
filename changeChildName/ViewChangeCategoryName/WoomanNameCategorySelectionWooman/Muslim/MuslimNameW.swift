...
struct MuslimNameW: View {
    
    @State var usersMuslimWooman: [User] = [
    ...
    private func getCardWidth(_ geometry: GeometryProxy, id: Int) -> CGFloat {
        let offset: CGFloat = CGFloat(usersMuslimWooman.count - 1 - id) * 10
    ...
    private func getCardOffset(_ geometry: GeometryProxy, id: Int) -> CGFloat {
        return  CGFloat(usersMuslimWooman.count - 24 - id) * 10 // можно менять расположение картинок на экарне, если поменять 8 на что то другое
    }
     
    private var maxID: Int {
        return self.usersMuslimWooman.map { $0.id }.max() ?? 0
    }
        ...
        VStack {
            GeometryReader { geometry in
                ...
                VStack(spacing: 24) {
                    Spacer(minLength: 80)
                    ZStack {
                        ForEach(self.usersMuslimWooman, id: \.self) { user in
                            // Range Operator
                            if (self.maxID - 3)...self.maxID ~= user.id {
                                CardView(user: user, onRemove: { removedUser in
                                    // Remove that user from our array
                                    self.usersMuslimWooman.removeAll { $0.id == removedUser.id }
                                })
...
struct MuslimNameW_Previews: PreviewProvider {
