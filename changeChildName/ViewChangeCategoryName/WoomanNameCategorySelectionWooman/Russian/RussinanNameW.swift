...
struct RussinanNameW: View {
    @State var usersNameWoomanRussian: [User] = [
    ...
    private func getCardWidth(_ geometry: GeometryProxy, id: Int) -> CGFloat {
        let offset: CGFloat = CGFloat(usersNameWoomanRussian.count - 1 - id) * 10
    ...
    private func getCardOffset(_ geometry: GeometryProxy, id: Int) -> CGFloat {
        return  CGFloat(usersNameWoomanRussian.count - 24 - id) * 10 // можно менять расположение картинок на экарне, если поменять 8 на что то другое
    }
     
    private var maxID: Int {
        return self.usersNameWoomanRussian.map { $0.id }.max() ?? 0
    }
    var body: some View {
        ZStack{
        ...
        VStack {
            GeometryReader { geometry in
                ...
                VStack(spacing: 24) {
                    Spacer(minLength: 80)
                    ZStack {
                        ForEach(self.usersNameWoomanRussian, id: \.self) { user in
                            // Range Operator
                            if (self.maxID - 3)...self.maxID ~= user.id {
                                CardView(user: user, onRemove: { removedUser in
                                    // Remove that user from our array
                                    self.usersNameWoomanRussian.removeAll { $0.id == removedUser.id }
                                })
...
struct RussinanNameW_Previews: PreviewProvider {
